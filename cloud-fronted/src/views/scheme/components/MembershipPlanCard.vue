<template>
    <div class="membership-card group relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-0.5 shadow-lg transition-all duration-500 hover:-translate-y-2 hover:shadow-xl mb-6 md:mb-0"
        :class="[
            specialClass,
            `transition-all duration-500 ease-out hover:shadow-lg hover:shadow-gray-200/50`,
        ...
        <div class="relative bg-white rounded-2xl overflow-hidden">
            ...
            <div class="p-4 md:p-6 pb-3 md:pb-4 text-center relative overflow-hidden" :class="headerBgClass">
                ...
                <!-- 推荐标记 -->
                <div v-if="isPopular" class="absolute top-2 right-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-yellow-300 animate-pulse"
            ...
            <div class="p-4 md:p-6 pt-3 md:pt-5">
                <button :class="buttonClass"
                    class="w-full rounded-lg py-2.5 md:py-3.5 text-sm md:text-base font-medium transition-all duration-300"
            ...
            <div v-if="plan.level === 'THIRD'"
                class="absolute -left-3 top-8 md:top-10 px-4 md:px-5 py-1 md:py-1.5 bg-gradient-to-r from-indigo-600 to-blue-700 text-white text-xs font-bold rounded-r-lg shadow-lg transform -rotate-90 -translate-x-7 md:-translate-x-8">
                顶级方案
            </div>
        </div>
    </div>
...
const transitionDelayClass = computed(() => {
    const delayValue = props.transitionDelay;
    if (delayValue === 0) return 'transition-delay-100';
    if (delayValue === 1) return 'transition-delay-250';
    if (delayValue === 2) return 'transition-delay-400';
    return '';
});
</script>